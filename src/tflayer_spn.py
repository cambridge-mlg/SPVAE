...
Sumweights have shape [1, num_product_child, K]
"""
...
class SumTFLayer(TFLayer):
    ...
    def build(self, productlayers):
        ...
        """
        num_product_child = productlayers.shape[1]

        if self.debug:
            self.W = tf.constant(0, name="weights",
                                 shape=[1, num_product_child, self.K],
                                 dtype=tf.float32,
                                 )  # shape = (1,num_product_nodes, K)
        else:
        ...
        self.log_p = self.W - tf.reduce_logsumexp(self.W, axis=[1])  # shape = (1,num_product_nodes, K)
        self.built = True

    def __call__(self, productlayers):
        """
        multiply weight matrix with child layer
        :param productlayers: list of `Tensor`s of shape == (batch, num_product_nodes)
        :return: a `Tensor` of shape == (batch, K)
        """
        with tf.variable_scope("SumLayer" + str(self.scope_id)):

            # this contains a copy operation !! slow!!
            productlayers = tf.concat(productlayers, axis=1) # shape == (batch, num_product_nodes)

            if not self.built:
                self.build(productlayers)

            # expand dimension so that multiple outputs are possible
            prods = tf.expand_dims(productlayers, axis=2)  # shape = (1, num_prod_nodes, 1)

            # exploit broadcasting
            out = prods + self.log_p  # shape = (1, num_prod_nodes, K)
            activation = tf.reduce_logsumexp(out, axis=[1], name="activation")  # shape == (batch, K)
            return activation
    ...
    def get_layer_entropy(self):
        log_p_bar = self.log_p - tf.expand_dims(tf.reduce_logsumexp(self.log_p, axis=[2]),axis=2)
...
def ProductOp(sumlayer1, sumlayer2, partition_id):
    ...
    with tf.variable_scope("ProdLayer" + str(partition_id)):
        ...
        batch_size = tf.shape(sl1)[0]
        n_nodes1 = int(sumlayer1.shape[1])
        n_nodes2 = int(sumlayer2.shape[1])

        activation = tf.reshape(batch_outer, [batch_size, n_nodes1 * n_nodes2],
                                name="activation")  # shape = (batchsize, num_prod_nodes)
