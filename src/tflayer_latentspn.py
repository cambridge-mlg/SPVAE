...
class LatentSumTFLayer(TFLayer):
    """
    # Use SumTFLayer instance as a function
    output = SumTFLayer(5,1234)(prodlayer_input)
    """

    def __init__(self, K, scope_id, zs, n_hidden=(2, 2)):
        """
        param K: number of nodes in SumLayer (integer)
        param scope_id: (integer)
        """
        super(LatentSumTFLayer, self).__init__(K, scope_id)
        self.zs = zs  # shape (k_samples, batch, nz)
    ...
    def build(self, productlayers):
        ...
        self.W = tf.reshape(gener(self.zs), [k_samples, batch_size,  nprods, self.K])
...
class LatentSP_TFLayerFactory(object):
    """
    ...
    def get_latentsumlayer_activation(self, scopeblock, K=None, zs=None):
        """
        ...
        # Sum Layer not yet constructed
            ...
            # evaluate sum layer
            sl = LatentSumTFLayer(K, scopeblock.id, zs=zs)
    ...
    def get_productlayer_activation(self, partitionblock):
        ...
        if partitionblock.id in self._productlayers_activation:
            ...
            sumlayer1 = self.get_latentsumlayer_activation(scopeblock1)
            sumlayer2 = self.get_latentsumlayer_activation(scopeblock2)
...
class LatentSPNTFLayer(TFLayer):
    """
    Usage:
    ...
    def __init__(self, K, scope_id, region_graph, leaflayer, sumK=3, leafK=3):
        ...
        self.splayer_factory = LatentSP_TFLayerFactory(self.LeafLayer)
        super(LatentSPNTFLayer, self).__init__(K, scope_id)
        ...
        for partitions_with_same_rank, scopes_with_same_rank in zip(partitions, scopes):
            ...
            for scope in scopes_with_same_rank:
                ...
                if len(scope.vars) == self.num_variables:
                    # (k_samples, batch, K)
                    sumlayer = self.splayer_factory.get_latentsumlayer_activation(scope, self.K)
                else:
                    sumlayer = self.splayer_factory.get_latentsumlayer_activation(scope, self.sumK)
