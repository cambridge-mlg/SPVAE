...
def three_TFrecord_datasets(data_dir, category, distribution_type, train_batch_size, take_frac=1.):

    if category < 0:
        category = "combined"

    train_ds = tf.data.TFRecordDataset(
        [os.path.join(data_dir, "mnist", distribution_type, str(category), "train.tfrecords")])
    validation_ds = tf.data.TFRecordDataset(
        [os.path.join(data_dir, "mnist", distribution_type, str(category), "validation.tfrecords")])
    test_ds = tf.data.TFRecordDataset(
        [os.path.join(data_dir, "mnist", distribution_type, str(category), "test.tfrecords")])

    if category == "combined":
        sizes = np.sum(np.load(os.path.join(data_dir, "mnist", "indices", "dataset_size.npy")),axis=0)
    else:
        sizes = np.load(os.path.join(data_dir, "mnist", "indices", "dataset_size.npy"))[category, :]
...
def three_npy_datasets(data_dir, category, distribution_type, train_batch_size, take_frac=1.):
    ...
    train_npy = np.load(os.path.join(data_dir, "mnist", distribution_type, str(category), "train.npy"))
    ...
    validation_npy = np.load(os.path.join(data_dir, "mnist", distribution_type, str(category), "validation.npy"))

    test_ph = tf.placeholder(name="test_data_ph", dtype=tf.float32)
    test_ds = tf.data.Dataset.from_tensor_slices(test_ph)
    test_npy = np.load(os.path.join(data_dir, "mnist", distribution_type, str(category), "test.npy"))
...
def three_constant_datasets(data_dir, category, distribution_type, train_batch_size, take_frac=1.):
    ...
    validation_ds = configure_constantds(validation_ds, sizes[1])
    test_ds = configure_constantds(test_ds, sizes[2])
