...
import numpy as np
"""
Various ways to prepare cifar10 tf.data.Dataset
"""
...
def three_ds(data_dir, category, distribution_type, train_batch_size, take_frac=1.):
    ...
    train_ds = tf.data.TFRecordDataset(
        [os.path.join(data_dir, "cifar10", distribution_type, "{}".format(category), "train.tfrecords")])
    validation_ds = tf.data.TFRecordDataset(
        [os.path.join(data_dir, "cifar10", distribution_type, "{}".format(category), "validation.tfrecords")])
    test_ds = tf.data.TFRecordDataset(
        [os.path.join(data_dir, "cifar10", distribution_type, "{}".format(category), "test.tfrecords")])

    # shape [category, (train size, validation size, test size)]
    if category == "combined":
        sizes = np.sum(np.load(os.path.join(data_dir, "cifar10",  "dataset_size.npy")),axis=0)
    else:
        sizes = np.load(os.path.join(data_dir, "cifar10", "dataset_size.npy"))
...
def three_npy_datasets(data_dir, category, distribution_type, train_batch_size, take_frac=1.):
    ...
    train_npy = np.load(os.path.join(data_dir, "cifar10", distribution_type, str(category), "train.npy"))
    ...
    validation_npy = np.load(os.path.join(data_dir, "cifar10", distribution_type, str(category), "validation.npy"))

    test_ph = tf.placeholder(name="test_data_ph", dtype=tf.float32)
    test_ds = tf.data.Dataset.from_tensor_slices(test_ph)
    test_npy = np.load(os.path.join(data_dir, "cifar10", distribution_type, str(category), "test.npy"))

    if category == "combined":
        sizes = np.sum(np.load(os.path.join(data_dir, "cifar10", "dataset_size.npy")),axis=0)
    else:
        sizes = np.load(os.path.join(data_dir, "cifar10", "dataset_size.npy"))[category, :]
